...
import { runOilAlertCheck } from "@/lib/alerts/oil-alert-runner";
...
async function handleRun(request: NextRequest, trigger: "cron" | "manual") {
  ...
  try {
    const result = await runOilAlertCheck(trigger, {
      dryRun: ["1", "true", "yes"].includes(request.nextUrl.searchParams.get("dryRun")?.toLowerCase() ?? ""),
    });
